...
INSERT INTO seasons (start_date, end_date, league_id) VALUES
  ('2018-03-21', '2018-06-21', 1), ('2019-03-20', '2019-06-20', 1);
...
SELECT p.name AS players, ga.date AS dates FROM players p
...
SELECT r.name AS referees, g.date AS dates, t1.name AS team1, t2.name AS team2
  FROM referees r JOIN game_referees gr ON r.id = gr.referee_id JOIN games g ON
  gr.game_id = g.id JOIN teams t1 ON g.team1_id = t1.id JOIN teams t2 ON
  g.team2_id = t2.id;

-- gets all of the start and end dates for the league
SELECT l.name AS league, s.start_date AS start_dates, s.end_date AS end_dates
  FROM league l JOIN seasons s ON l.id = s.league_id;

-- orders the teams by how many goals they made
